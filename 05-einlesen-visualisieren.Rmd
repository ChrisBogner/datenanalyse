...
Ein kurzer Blick auf den Datensatz. Hierbei handelt es sich um Daten zu Treibhausgasemissionen auf der EU-Ebene, die ich bei [eurostat](https://appsso.eurostat.ec.europa.eu/nui/show.do?dataset=env_air_gge&lang=en) heruntergeladen und vorgefiltert habe. Die Datenbank bietet sehr viele Datensäzte und ist als Quelle für Berichte hervorragend geeignet `r emo::ji('smile')`.
...
## Fehlerbalken und Co.
Um die Variabilität der Daten grafisch darzustellen, bieten sich Fehlerbalken, Bereiche etc. an. Dafür hat `ggplot2` spezielle geoms. Hier ein Beispiel aus dem `ggplot2` Buch @Wickham2020.
```{r, out.width = "90%"}
y <- c(18, 11, 16)
...
Sie sehen, dass man `ggplot` Objekte so wie andere Objekte in R zuweisen kann, um mit ihnen später zu arbeiten. In diesem Fall ist `base` ein `ggplot` Objekt.
```{r}
class(base)
```
...
Um mehrere Grafiken nebeneinander zu plotten, nutzen wir die Funktion `grid.arrange()` aus der Bibliothek `gridExtra`. Um sie zu nutzen, müssen wir die `ggplot` Objekte abspeichern und dann mit Hilfe der Funktion `grid.arrange`, wie der Name schon sagt, "arrangieren". In diesem Fall wollen wir in der Gesamtgrafik eine Zeile (`nrow = 1`), sodass die Grafiken nebeneinander stehen.
...
Um die Grafiken untereinander abzubilden, bestellen wir entsprechend zwei Zeilen.

```{r, out.width = "90%"}
alles <- grid.arrange(p1, p2, nrow = 2)
```
...
```{r}
ggsave(filename = 'Fehlerbalken.pdf', plot = alles, device = 'pdf', width = 7, height = 5)
```
...
grid.arrange(bet, zweit, nrow = 2, heights = c(3, 3))
```
