# Stichproben und Variabilität

```{block, type = 'rmdoutcomes'}
- Begriffe Stichprobenverteilung und Standardfehler erkären
- Zufall bei wiederholter Erstellung von Stichproben erkennen
- Stichprobenverteilung darstellen
- Einfluss der Stichprobengröße auf Stichprobenverteilung bennen
```

Mit diesem Kapitel steigen wir ein in die schließende Statistik. Wir beginnen damit, wie man in der Statistik zu Daten kommt und welche Rolle der Zufall dabei spielt. Dabei konzentrieren wir uns auf die Erhebung von Stichproben, nicht auf das Designen von Experimenten. Das ist eine Kunst für sich und geht über die Ziele dieses Kurses hinaus.
...
Wir nutzen wieder die selbst erstellten Daten aus der Aufgabe \@ref(werdeschlau). Die 1200 Studiere sind unsere **Grundgesamtheit**. Da wir die Daten selbst erstellt haben, wissen wir alles über sie. Das ist ein großer Vorteil von Computerexperimenten `r emo::ji('smile')`. Damit alle dieselben Daten erstellen, ist die Zeile `set.seed(123)` sehr wichtig. Sie sorgt dafür, dass der Generator für Zufallszahlen in einen zufälligen, aber reproduzierbaren Zustand versetzt wird. DIe Zahl in den Klammern ist nicht wichtig. Wichtig ist, dass alle dieselbe benutzen `r emo::ji('smile')`.
...
Wir wollen nun wissen, wie viele Studierende unter den Befragten in der Stadt und auf dem Land wohnen.
...
befragung %>% 
  ...
  count()
```
Das Ganze möchten wir als Anteile ausdrücken. Die Funktion `n()` kann innerhalb der Funktion `summarize()` zum Auszählen genutzt werden. Wir teilen durch die Stichprobengröße.
```{r}
befragung %>% 
  group_by(wohnort) %>% 
...
befragung_reps
```
Jetzt wird uns angezeigt, dass es in dem Datensatz `befragung_reps` 33 replicates (Wiederholgungen) gibt. Diese sind einfach nacheinander in `befragung_reps` angeordent (blättern Sie durch den Datensatz). Dementsprechend hat der Datensatz 50 $\times$ 33 = 1650 Zeilen.
...
Erwartungsgemäß bringt jede Wiederholung der Befragung, die wir ja als zufälliges Herausgreifen der Studierenden ohne Mehrfachbefragung programmiert haben, etwas andere Ergebnisse (sehen Sie sich die `studnet_id` in den replicates an!). In einem Histogramm sieht das ganze so aus:
...
Was passiert, wenn wir unsere Umfrage nun 1000 Mal wiederholen? Wir können den ganzen Code recyclen und müssen nur die reps entsprechend verändern. Wir erstellen dafür nun eine extra Variable `befragung_reps`. Beim Histogramm sollten wir die `binwidth` etwas heruntersetzten, da wir jetzt sehr viel mehr Daten haben und diese detaillierter anzeigen lassen können. Zusätzlich wird die x-Achse "frei" gegeben, i.e. die Skalierung wird jetzt auf jeder Achse separat bestimmt `scales = 'free_x'` in `facet_wrap()`, um die Verteilungen besser zu sehen.
...
Die Histogramme geben nun sehr gut die Verteilung der Anteile der Stadt- und Landbewohner wieder. Solche Verteilungen nennt man **Stichprobenverteilungen**. Sie geben die Verteilung einer statistischen Kenngröße (Statistik), in unserem Fall Anteil, wider.
...
```
Wir stellen die drei Stichprobenverteilungen dar. Die binwidth ist jeweils eine andere.
...
Sei sehen, dass mit steigender Stichprobengröße der Standardfehler sinkt. Das ist intuitiv verständlich, denn je mehr Studierende wir (pro Wiederholung!) befragen, desto repräsentativer ist die Stichprobe.

Da dieses Kapitel so wichtig ist, gibt es Ausnahmesweise vorformulierte Take-Home-Messages `r emo::ji('smile')`:
...
- Die Verteilung einer statistischen Kenngröße, die aus Stichproben ausgerechnet wurde, heißt Stichprobenverteilung. Um diese Verteilung zu bekommen, muss man wiederholt Stichproben erheben. Je mehr Stichproben man erhebt, desto genauer kann man die Stichprobenverteilung beschreiben.
...
### Lohnt eine Station zum Ausleihen von Farrähdern?
In unserem fiktiven Beispiel der Uni Werdeschlau geht es eigenlich darum, ob sich eine Station zum Ausleihen von Fahrrädern lohnen würde. Daher ist die Frage interessant, wie viele Studierende mit dem Fahrrad zur Uni kommen. Wiederholen Sie die obige Analyse und ermitteln Sie statt der Anteile von Stadt- und Landbewohnern nun die Anteile der unterschiedlichen Verkehrsmittel. Nutzen Sie für die flexible Beprobung den folgneden Code:
```{r}
...
In der Lösung zu dieser Aufgabe erhalten Sie auch weiter Tipps zur Darstellung von Histogrammen und Dichtefunktionen `r emo::ji('sunglasses')`.


## Ihre Arbeit einreichen
- Speichern Sie Ihr Notbook ab und laden Sie nur die .Rmd Datei vom Server.
