...
Im letzten Kapitel haben Sie die einfache Normalregression kennen gelernt. Bevor Sie die geschätzten Parameter und deren Konfidenzintervalle interpretieren, müssen Sie die Annahmen der Normalregression überprüfen (vor allem grafisch):
...
Eine sehr oft verwendete Methode, die Modellparameter zu schätzen, heißt **Methode der kleinsten Quadrate**. Sie beruht auf der Idee, dass diejenigen Modellparameter, $y$-Achsenabschnitt und die Steigung, die besten sind, bei denen die Summe der quadrierten Residuen die kleinste ist. Formal schreibt man:
...
Die geschätzten Modellparameter sind **Zufallsvariablen**. Unter der Annahme, dass die Residuen nomralverteilt sind:
...
Die $\mathit{SQE}$ beschreibt die Variation der angepassten Werte um den Mittelwert und $\mathit{SQR}$ entspricht dem nicht erklärten Teil der Streuung. Das heißt, dass je kleiner die Residualstreuung, desto besser das Modell, weil es dann einen größeren Anteil der Streuung der Daten erklären kann. Das Verhältnis der Gesamtstreuung zur Residualstreuung nennt man das **Bestimmtheitsmaß** (oder auch Determinationskoeffizient). Dieser wird meistens mit $R^2$ bezeichnet und ist definiert als:
...
- `mse`: mittlerer quadratischer Fehler `mean(residuals(lin_mod)^2)`
- `rmse`: Wurzel aus `mse`
...
Wir stellen die Ergebnisse wie gewohnt dar.
```{r, out.width = '90%'}
visualize(bootstrap_distn_slope) +
    shade_confidence_interval(endpoints = percentile_ci) 
```

Verglichen mit dem Konfidenzintervall basierend auf der Normalverteilungsannahme, ist Bootstrap sehr ähnlich. Das liegt daran, dass die Normalverteilungsannahme ja erfüllt ist. In so einem Fall sind die Konfidenzintervalle sehr ähnlich.
```{r}
...
class(my_bootstraps)
```
Wir sehen uns so eine Bootstrap-Stichprobe mal an. Dazu nutzen wir die Funktion `analysis()`, die solche *splits* richtig behandelt. Jede Bootstrap-Stichprobe ist durch Ziehen mit Zurücklegen aus dem Datensatz `befragung` hervorgegangen.
...
### Schritt 1: Modell auf den Bootstrap-Stichproben anpassen
...
# Standard-Konfidenzintervalle mit der Annahme der Normalverteilung der Residuen
...
Wir plotten alle Ergebnisse nun zusammen. Das müssen wir "händisch" machen, da uns ja nicht die tolle Funktion `visualize()` aus `infer` zur Verfügung steht. Wir fügen die Konfidenzintervalle als vertikale Linien zu den Histogrammen hinzu.
...
In diesem Beispiel ähneln sich die Konfideznintervalle, da die Annahmen der Normalregression ja erfüllt sind. Das wird in der Aufgabe, die Sie selbständig bearbeiten werden, nicht mehr der Fall sein. Selbstverständlich sollen Sie im Falle von erfüllten Annahmen einfach die Standardkonfidenzintervalle nutzen und brauchen kein Bootstrap. Sie werden aber sehen, dass eim Modellieren mit richtigen Daten die Annahmen der Normalverteilung und der Homoskedastizität sehr häufig verletzt sind. Mit dem Bootstrap sind Sie jetzt dafür gerüstet `r emo::ji('smile')`.
...
Wiederholen Sie die obere Aufgabe nun mit Hilfe der Bibliothek `rasample`. Tip: Wandeln Sie die Inselnamen, die nur als Zeilennamen existieren, in eine richtige Spalte um `gala <- gala %>%
