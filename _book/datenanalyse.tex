...
\hypertarget{high-level-funtion-plot-und-low-level-funtion-lines}{%
\subsection{\texorpdfstring{High-level-Funtion \texttt{plot} und low-level-Funtion \texttt{lines}}{High-level-Funtion plot und low-level-Funtion lines}}\label{high-level-funtion-plot-und-low-level-funtion-lines}}
...
Uns interessieren hier nur die Spalten MO\_TT, MO\_TN, MO\_TX und MESS\_DATUM\_BEGINN. Das sind jeweils die Monatsmittel der Lufttemperatur in 2m Hoehe, Monatsmittel des Lufttemperatur Minimums, Monatsmittel des Lufttemperatur Maximums und Beginn der Messperiode. Um die Daten als Zeitreihen darstellen zu können, wandeln wir die Spalte MESS\_DATUM\_BEGINN in ein richtiges Zeitpobjekt. Das geht mit der Funktion \texttt{as.Date}.
...
Es sind Daten von Oktober 2018 bis April 2020. Die Funktion \texttt{plot} stellt erstellt ein Streudiagramm. Mit den Parametern \texttt{xlab} und \texttt{ylab} lassen sich die beiden Achsen beschriften und \texttt{main} fügt einen Titel dazu. Der Parameter \texttt{type} betsimmt die Wahl der Symbole; hier benutzen wir \texttt{type\ =\ b} für \emph{both}, also sowohl Punkte als auch Linien.
...
Die wichtigsten high-level-Grafikfuntionen \citep[nach][verändert]{Ligges2008}:
...
\midrule
\endhead
\texttt{plot} & kontextabhängig -- generische Funktion mit vielen Methoden\tabularnewline
\texttt{barplot} & Stabdiagramm\tabularnewline
\texttt{boxplot} & Boxplot\tabularnewline
\texttt{contour} & Höhenlinien-Plot\tabularnewline
\texttt{coplot} & Conditioning-Plots\tabularnewline
\texttt{curve} & Funktionen zeichnen\tabularnewline
\texttt{dotchart} & Dotplots (Cleveland)\tabularnewline
\texttt{hist} & Histogramm\tabularnewline
\texttt{image} & Bilder (3. Dim. als Farbe)\tabularnewline
\texttt{mosaicplot} & Mosaikplots (kategorielle Daten)\tabularnewline
\texttt{pairs} & Streudiagramm-Matrix\tabularnewline
\texttt{persp} & perspektivische Flächen\tabularnewline
\texttt{qqnorm} und \texttt{qqplot} & QQ--Plot\tabularnewline
\bottomrule
\end{longtable}

Die wichtigsten low-level-Grafikfuntionen \citep[nach][verändert]{Ligges2008}:
...
\texttt{polygon}\strut
\end{minipage} & \begin{minipage}[t]{0.56\columnwidth}\raggedright
(ausgefullte) Polygone\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.38\columnwidth}\raggedright
\texttt{pretty}\strut
\end{minipage} & \begin{minipage}[t]{0.56\columnwidth}\raggedright
berechnet ``hubsche'' Einteilung der Achsen\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.38\columnwidth}\raggedright
...
Zur Vertiefung dieses Kapitels, lesen Sie \citet{Ligges2008}, Kapitel 8.1.3.
...
\begin{figure}
\includegraphics[width=1\linewidth]{Regionen_und_Raender} \caption{Aufteilung der Graphikfläche [@Ligges2008].}\label{fig:aufteilung}
\end{figure}

Mit der Funtion \texttt{par} lassen sich sehr viele Einstellung der Grafik verändern. Viele Einstellungen übergibt die Funtion \texttt{plot} selbständig an \texttt{par}, z.B. \texttt{log} (Logarithmieren der Achsen), \texttt{cex} (Größe eines Punkts) oder \texttt{col} (Farbe). Andere können aber nur durch Aufrufen der Funtion \texttt{par} verändert werden. Dazu gehören die inneren Ränder \texttt{mar} und die äußeren Ränder \texttt{oma}, die Aufteilung der Grafikebene mit \texttt{mfrow} oder \texttt{mfcol}.
...
\includegraphics{datenanalyse_files/figure-latex/unnamed-chunk-26-1.pdf}
...
\texttt{cex} & Gröoße eines Punktes bzw. Buchstaben\tabularnewline
...
\texttt{main} & Uberschrift\tabularnewline
\texttt{mar} & Größe der inneren Ränder fur Achsenbeschriftung etc.\tabularnewline
\texttt{mfcol}, \texttt{mfrow} & mehrere Grafiken in einem Bild\tabularnewline
\texttt{pch} & Symbol fur einen Punkt\tabularnewline
\texttt{type} & Typ (l=Linie, p=Punkt, b=beides, n=nichts)\tabularnewline
...
\hypertarget{inhalt-der-live-einfuxfchrung-und-der-uxfcbungen-2}{%
\section{Inhalt der live Einführung und der Übungen}\label{inhalt-der-live-einfuxfchrung-und-der-uxfcbungen-2}}
...
\caption{\label{tab:unnamed-chunk-28}Ars Mensaausgaben}
\centering
\begin{tabular}[t]{lr}
\toprule
Wochentag & Ausgaben\\
...
Bauen Sie die Grafike aus der Einführung nach (Abbildung \ref{fig:wahlbeteiligung}).
...
\tightlist
\item
  Lesen Sie den Datensatz \texttt{Wahlbeteiligung.csv} in R ein und ordenen Sie ihn dem Objekt \texttt{bet} zu. Der Datensatz hat einen header und haben einen Strichpunkt als Spaltentrenner.
\item
...
Bauen Sie die Grafike aus der Einführung nach (Abbildung \ref{fig:zweitstimme}).
...
\tightlist
\item
  Lesen Sie den Datensatz \texttt{Wahlbeteiligung.csv} in R ein und ordenen Sie ihn dem Objekt \texttt{zweit} zu. Der Datensatz hat einen header und haben einen Strichpunkt als Spaltentrenner.
\item
  Sehen Sie sich die Struktur und die ersten und letzten 6 Zeilen des Datensatzes an.
\item
...
Stellen Sie beide Grafiken nebeneinander dar wie in Abbildung () gezeigt
...
Bei Berichten ist es oft störend, wenn Grafiken einen übermäßig großen weißen Rand haben.
...
\hypertarget{spielen-mit-der-funtion-par}{%
\subsection{\texorpdfstring{Spielen mit der Funtion \texttt{par}}{Spielen mit der Funtion par}}\label{spielen-mit-der-funtion-par}}
